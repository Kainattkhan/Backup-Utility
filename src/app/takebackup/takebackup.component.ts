      ...
      if (data) {   
        ...
        this.results = this.dataBackup.map((dropdownData: any) => {
          return {
            option: this.selectedOption === "option1"?dropdownData.Database:dropdownData.database,
            result: dropdownData.Date,
            downloadLink: this.selectedOption === "option1"?`http://localhost:8080/mongo/zip/${dropdownData.Date}/${dropdownData.Database}`:`http://localhost:8080/sql/createzip/${dropdownData.Date}/${dropdownData.database}`
          }
        });
      }
    });
